...
GaugeChart.propTypes = {
  ...
  diffColor: _propTypes.default.func,
  diff: _propTypes.default.number,
  formatDiffValue: _propTypes.default.func
...
var addText = function addText(percentage, props, outerRadius, width, g) {
  ...
  if (!props.diff) {
    ...
    if (icon !== '') {
      ...
      newElem.append('text').text(formatDiffValue ? formatDiffValue(diff) : Math.abs(diff)).attr('transform', "translate(".concat(outerRadius.current + width.current / 11 / (text.length > 10 ? text.length / 10 : 1) + width.current / 11 / (text.length > 10 ? text.length / 10 * 2 : 1 * 2), ", ").concat(outerRadius.current / 2 + textPadding, ")")).style('font-size', function () {
        return "".concat(width.current / 11 / (text.length > 3 ? text.length / 2 : 2), "px");
...
var calculateRadius = function calculateRadius(width, height, outerRadius, margin, g) {
  ...
  console.log(height, width.current); //First check if it is the width or the height that is the "limiting" dimension
...
var updateDimensions = function updateDimensions(props, container, margin, width, height) {
  ...
  margin.current.bottom = divHeight * marginInPercent; // height.current = divHeight
